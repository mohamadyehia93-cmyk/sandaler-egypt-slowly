...
const MyCollections = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(lang === "ar" ? "حذف هذه المجموعة؟" : "Delete this collection?")) return;
      ...
      <div className="px-4 py-5 space-y-3">
        {!user ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-muted-foreground">{lang === "ar" ? "لا توجد مجموعات بعد" : "No collections yet"}</p>
          </div>
        ) : (
          items.map((e) => (
            <div key={e.id} className="bg-card rounded-xl shadow-card p-3 flex gap-3 items-center">
              ...
              <button onClick={() => navigate(`/dashboard/culture-actor/edit-collection/${e.id}`)} className="p-2 rounded-lg bg-role-culture-actor/10 text-role-culture-actor" aria-label={lang === "ar" ? "تعديل" : "Edit"}>
