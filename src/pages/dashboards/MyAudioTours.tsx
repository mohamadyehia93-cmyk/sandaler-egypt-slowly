...
const MyAudioTours = () => {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["my-audio-tours", user?.id],
    enabled: !!user,
    queryFn: async () => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(lang === "ar" ? "حذف هذه الجولة؟" : "Delete this tour?")) return;
    const { error } = await supabase.from("audio_tours").delete().eq("id", id);
    if (error) return toast.error(error.message);
    toast.success(lang === "ar" ? "تم الحذف" : "Deleted");
    queryClient.invalidateQueries({ queryKey: ["my-audio-tours"] });
    queryClient.invalidateQueries({ queryKey: ["audio-tours"] });
  };
      ...
      <div className="px-4 py-5 space-y-3">
        {!user ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-muted-foreground">{lang === "ar" ? "لا توجد جولات بعد" : "No tours yet"}</p>
          </div>
        ) : (
          items.map((e) => (
            <div key={e.id} className="bg-card rounded-xl shadow-card p-3 flex gap-3 items-center">
              <div className="w-16 h-16 rounded-lg overflow-hidden bg-secondary shrink-0 flex items-center justify-center">
                {e.image ? <img src={e.image} alt="" className="w-full h-full object-cover" /> : <Headphones className="w-6 h-6 text-muted-foreground" />}
              </div>
              ...
              <button onClick={() => navigate(`/dashboard/culture-actor/edit-tour/${e.id}`)} className="p-2 rounded-lg bg-role-culture-actor/10 text-role-culture-actor" aria-label={lang === "ar" ? "تعديل" : "Edit"}>
