...
/** Only the app's current language is mandatory — see StepTitle. */
const StepDescription = ({ form, set }: Props) => {
  ...
  const arField = (
    <div key="ar">
      <label className={labelClass}>
        ...
        {ar ? "الوصف بالعربية *" : "Description (Arabic) (optional)"}
      </label>
      <textarea
  ...
  const enField = (
    <div key="en">
      <label className={labelClass}>
        <FileText className="w-3.5 h-3.5 text-role-service-provider" />
        {ar ? "الوصف بالإنجليزية (اختياري)" : "Description (English) *"}
      </label>
      <textarea
        className={`${inputClass} min-h-[120px] resize-none`}
